...
- `w0bqp0RaeD` (125 funcs): **paquete de detección AC — inmediatamente anterior a `main` en pclntab**. 11 init closures. Función crítica `TnC04uk4`: 8 closures directas + `func6.1-16` + `func6.dhCJhjGa.16` + `gowrap1-6` = **6 goroutines lanzadas**. Funciones complejas: `QKaVdXPL` (8 closures), `aiV8LM9eoaot` (5), `bxCiu5XSx1` (5), `OY6DwAo` (5), `rut8pO1Cygha`. 2 tipos comparables: `RIpbVXrKn` y `O1pxFVY` (usados como map keys). Sin `decFunc` (strings cleartext). Cadena pclntab: `GTShob4MX` (registry) → `T1XTTlVrw4yb` → `w0bqp0RaeD` → `main`
- `lI61JSyj` (30 funcs): ~10 init closures, adyacente a `HEirZZGS` (error types) → posible paquete helper con strings cifrados
- `DQTJuVp_jZ` (115 funcs confirmadas): posición en pclntab entre `E5D2QU` (`internal/poll`) y `u26yruH2Ly_u` (`net` stdlib). Sin `decFunc`. **56 init closures** incluyendo `init.AAaUijjhdsR.func61/62` (función nombrada inner en init, igual a `ZrMEw4hJW` de main) y `init.GFwvMRC3zna[go.shape.bool].func57-60` (función genérica `bool`). 9 tipos comparables: `E5Cq49a`, `NgA3XBFdmdb`, `OK6p4yyx`, `SgNrJN`, `TB3Q9SaVnrtE`, `TKS90l`, `VmoFu3c`, `XvC0liM9`, `ZHTCBMPJZQf`. Vecinos en area de memoria: `reflect`, `internal/bytealg`, `internal/runtime/atomic`, `bVtNY51`, `f3SkGnYxZwK`, `GYOujgH`, `HQCYVEFsqR`. `atomic.(*Pointer[go.shape.func(string) func()]).Store` = registro de handlers tipo `func(string) func()` vía atomic. Los 56 init closures registrando handlers + reflect + posición pre-net sugiere: **`github.com/go-playground/validator/v10`** (registra 50+ validators en init usando reflect) o stdlib `net/netip` (9 tipos IP comparables)
