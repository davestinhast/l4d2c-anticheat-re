...
- `DQTJuVp_jZ` (109 funcs confirmadas): posición en pclntab entre `E5D2QU` (`internal/poll`) y `u26yruH2Ly_u` (`net` stdlib). Sin `decFunc`. **72 init closures** incluyendo `init.AAaUijjhdsR.func61/62` (función nombrada inner en init), `init.func56.deferwrap1` (recurso del SO adquirido en init!), y `init.func15.1/21.1/24.1/25.1/37.1/38.1/45.1/51.1/54.1/56.1` (múltiples sub-closures). Función libre `zx58WgqurVtf` (func1 + func1.deferwrap1). 9 tipos comparables como map keys: `E5Cq49a`, `NgA3XBFdmdb`, `OK6p4yyx`, `SgNrJN`, `TB3Q9SaVnrtE`, `TKS90l`, `VmoFu3c`, `XvC0liM9`, `ZHTCBMPJZQf`. **Sin pointer receivers en pclntab** (0 métodos visibles en pclntab). Vecinos: `reflect` y runtime en contexto cercano. `init.func56.deferwrap1` = un init closure abre/usa un recurso del SO via defer (muy inusual, descarta validator puro). Hipótesis actual: **SIN CONFIRMAR** — podría ser un paquete de crypto/encoding con inicialización de hardware (AES-NI probe, etc.) o un paquete de registro de handlers con cleanup. **NOTA:** `Ve0nLKBMzmC` ya confirmado como `validator/v10`
...
| `buwDgX7Zkn` | **Wails CORS/Security middleware** — validación de origen para WebView2 | **10 funciones**. Receptor único `Dqko6vxmobF6` con métodos preservados: **`GetOriginFromURL`** (3 closures + func3.1 = extrae origen scheme+host+port de una URL), **`IsOriginAllowed`** (valida si el origen está permitido = whitelist CORS), `fAhY134`, `mMHHzf`, `t7ZmFssa` = helpers. Función libre `G1JsQENk`. Sin `decFunc`. Posición: DESPUÉS de `VKiZI7` (WebView2 COM), ANTES de `NDWN1oE3`. Valida que los requests al WebView2 provengan de orígenes autorizados |
| `NDWN1oE3` | Paquete Wails/UI adyacente a buwDgX7Zkn | Función `E0FDAlU` visible. Aparece entre `buwDgX7Zkn` y `h7lxVr`. No analizado en profundidad |
...
| `Plcr2cx` | **AC crypto utility** — capa de criptografía del AC, adyacente a d8a0oX50i y math/big | **37 funciones**. Función principal `SVJaXu` (func1/2/3/3.1/4/4.1 = 6 closures con sub-lógica). `vHCFkvZ1I.deferwrap1` = recurso del SO adquirido/liberado. Sin `decFunc`, sin init closures, sin pointer receivers. Funciones: AlOjYk, Bd0Anencw, CgWgrWQ, DdB2QaI9Tr, ez8rbI, FkyA1t, hEQiyR8, hpszndafv, i7Vz_NFRnC, iLdkQrYCEy, IUcsAaU, Kg1F6_Y, kzhep6dsNGu, MoOBpYyR, NCN86aa, np7ujWi3L, PJJeJOAbiMG, QvfpqKW, Re6GCflna9, rHQg4df6V, S5uOoca1, TdRiPFQ, tXwYfW7C4o, uox8PiIVB, wTmsnvptZn2, ZnC4xHv. Posición: DESPUÉS de `d8a0oX50i` y `math/big`, ANTES de `H5_Lib8AHNQN` (Wails runtime). Probablemente: key derivation, firma digital o hashing para tokens de sesión |
| `i7OyDUpCDA3q` | **AC file utility** — utilitario de archivos/handles adyacente a main.QSUMsCa | **12 funciones**. `AT8OyBIVl` = tipo comparable (map key). Función principal `QjLFyyZAL`: func1/2/3 + **deferwrap1** + **deferwrap2** (2 recursos del SO liberados via defer!) + función interna nombrada `gxjdhFSmi` (func4 = análisis de resultado). Funciones libres: `D5UnAj`, `D9QABBJhz`, `gxjdhFSmi`, `init`. Sin `decFunc`. Posición: ANTES de `main.QSUMsCa.gowrap4/3/2` (scanner VPK). `QjLFyyZAL` con 2 deferwraps = abre 2 handles de archivos/directorios para el scan |
...
| `jsD5qQ4` | **Micro-paquete utilitario** adjacent a `LbVoc9TjklSB` (net/netip) | **8 funciones**. Sin init closures, sin `decFunc`. Funciones: `aP3e5QYtu`, `azwenqBD` (func1), `gYzvEjPWYt`, `h_9rQ0`, `hn__W6zm`, `lMSirSG9`. Posición: inmediatamente ANTES de `LbVoc9TjklSB.(*G_V9mRiw).AppendTo/As16/As4` en la type table. Probable dependencia interna de net/netip o utility de IP parsing. `azwenqBD` con 1 closure = función principal |
...
| `Q2v0Ja734` | **Probable `hash/fnv` o `hash/crc64`** — hash de 64 bits (mínimo) | **3 funciones** únicamente. `(*q3FlbqGG).Sum64` + `(*q3FlbqGG).Write` = implementa `hash.Hash64` interface. Función libre `Mq0Ozt0EZPQ` = probablemente el constructor `New64()` o `New64a()`. Posición: entre `F5ImErztW` e `i7OyDUpCDA3q`. Contexto cercano incluye `ZjyFEdOLj.aJ6HgUqCH.PutUint64` (binary.LittleEndian) — se usa junto a encoding binario |
| `sLqxHdT` | **Probable `golang.org/x/text` interno** o AC custom (bytes util cifrado) | **36 funciones**. `decFunc` PRESENTE (strings cifrados). Tipos: `cChjCnESm5`, `jK8057_` (usado en generics de ge3tkaLtE como `[]jK8057_`), `n4RzlLuBp4GM`. Función `aFoLnPeIZXz` (mismo nombre que bytes.Buffer en gF8Cxkh_m — visible en type table como dependencia). Sin init closures. Posición en type table: adyacente a `hRyHlb` (x/text/internal/number). Probablemente dependencia interna de golang.org/x/text |
...
| `i7OyDUpCDA3q` | **AC file utility** — utilitario de archivos/handles adyacente a main.QSUMsCa | **12 funciones**. `QjLFyyZAL` (func1/2/3 + **deferwrap1** + **deferwrap2** + `gxjdhFSmi.func4` = 2 COM/handles liberados). `D5UnAj`, `D9QABBJhz`, `gxjdhFSmi`. Tipo comparable `AT8OyBIVl`. Sin `decFunc`. Posición: ANTES de `main.QSUMsCa.gowrap4/3/2` = utilitario del scanner VPK |
| `aHBivM89uLf` | **Probable capa de transporte gRPC o AC** | **104 funciones**, `decFunc` PRESENTE. Tipos: `Bm44nr`, `ZJxS59`, `Cvo_UA_gdU`, `tkUjho`, `UIn9hT`, `OZ2Af5`, `aacvKl`, `vYa3Hy`, `j76Gxb`, `wFoOPt`, `hn0dX7`, `ev8R6e`, `S9Q5aV`, `gYz88H`. Firma clave: `func(Cvo_UA_gdU, ZJxS59, uint32, []uint8) error` = función de envío/escritura. Contexto: adyacente a `eUmM3IpzIrV` (connectivity layer) con structs `z28Msh9rGNh`, `quDjcDjdi`, `JC1j0AALEr5`. Strings cifrados = lógica sensible de transporte |
...
| `keNa_4m` | **Módulo de sesión/autenticación del AC** | **29 funciones**. Tipo `pvusDPtY5vCp` (4 métodos: `_GoIAD`, `evojx0Jc`, `fkQDlPvKUQ`, `h7OdHS4`). 23 funciones libres: `_1U_JVgZKyap`, `A19Yd7rFZH_`, `AAs7V4vmMZA`, `aKrHepf5N`, `cfcL4ca2le`, `cQ6WixJ`, `dkV6l6kGaQ`, `DPCCiaoS`, `EBg3Hlso`, `eQHF1TaraQ`, `HMGIADKRc`, `i3ZBglLDtm`, `IUJ55okf`, `JrOHXeMX6vuW`, `ljjjRHs`, `Lnr6NFAWAA`, `MKLEUUa6F`, `P1kLEFFzQG`, `QEap9S1lpO`, `TAZv8E`, `uE66JFzHSqr`, `uMeiix9DkjT`, `zIoFEdU9`. 1 init closure. Sin `decFunc`. Posición: ANTES de `AnNE8Dn7`. El método `_GoIAD` = probable "GoID/GoAhead" para autorización de sesión |
...
| `lI61JSyj` | **Probable `google.golang.org/grpc/codes`** — status codes de gRPC | **30 funciones**. **18 init closures** (func1-func14 + sub-closures: func5.1, func7.1, func9.1, func11.1) = patrón de registro de 17+ códigos de estado gRPC. Sin `decFunc`. Funciones libres: `A2Wb5kgV`, `b6flaJ2cFv`, `bWoJkWs5Y0E`, `eyGR92pY`, `gca01ckHo4`, `J0leaMa26e3j`, `pCg2nu`, `SaHHfJIUCC`, `xBPWjcvJqJU`, `Zlac9KtsLC`, `zRD1vhhf816q` (11 funciones = probablemente Code.String(), FromInt(), etc.). Sin pointer receivers. Posición EXACTA: DESPUÉS de `HEirZZGS` (error types del AC), ANTES de `WU2ixCjkiE` (x/net/http2). gRPC status codes: OK(0), Canceled(1), Unknown(2), InvalidArgument(3), DeadlineExceeded(4), NotFound(5), AlreadyExists(6), PermissionDenied(7), ResourceExhausted(8), FailedPrecondition(9), Aborted(10), OutOfRange(11), Unimplemented(12), Internal(13), Unavailable(14), DataLoss(15), Unauthenticated(16) = 17 códigos + extras = ~18 closures |
...
| `HEirZZGS` | **Error types** del AC | **22 funciones**. Tipos: `FCYxaSy52` (método `dusywnY16JVL`); `fiAWx26dm` (Error, Error.func1, Error.func1.1); `fVlYVq` (dMd_7D9r, jZ_9nsIr); `gTfmQ8uFof` (pUNtk9w); `kDhvVxE` (cs7fCMDFJVQ2, lazCKwvQKJGX, pbTZTWd7, v_lQbk = 4 métodos); `ofV3sGd` (Error, Error.func1). Funciones libres: `gtg1IURVhX`, `kQqBDf8WVzOM`, `xwUR_dJFTF`. 2 init closures. Sin `decFunc`. Posición: DESPUÉS de `fm9Lu8y5M2` (x/text/transform), ANTES de `lI61JSyj` (grpc/codes). Paquete de definición de errores y tipos de estado custom del AC |
...
| `anGbYY2EWPw` | **314** | **CONFIRMADO: probable `google.golang.org/protobuf/internal/filedesc`** — registro masivo de descriptores proto | **301 init closures** (no 249+ — conteo exacto). **Un solo tipo receptor `HEGKR0lOPr3`** con 7 métodos: `bXSw1yQa`, `f7joP2b7CkWE` (3 closures: func1/2/3), `kEQnsN`, `mipEEqS`, **`Parent`** (preservado = `protoreflect.Descriptor.Parent()`), **`String`** (preservado). Función libre `Ep_prp` (deferwrap1 = limpieza de recurso). Función `tKxOT0Tgss` = utilidad interna. Sin `decFunc`. `HEGKR0lOPr3` = `filedesc.File` (o `filedesc.Desc` base). 301 init closures = 301 descriptores de archivos `.proto` registrados en init() del AC — incluyendo todos los archivos proto de gRPC, googleapis, y los propios del servidor L4D2Center. Posición: DESPUÉS de `YCJ5PUz_M` (firmas 4), ANTES de `vypSO2` (Wails assetserver) |
...
| Paquete | Funciones | Observación |
|---------|-----------|-------------|
| `GtN_WZiIQ` | **18** | **Probable `google.golang.org/grpc/internal/status`** — status/error de gRPC | Dos tipos error: `dpN7za` (Error, Unwrap = wrapping error), `EKV_aagFA` (Error, Error.func1 = simple error). Tipo comparable `hik61_SamaY` (usado como map key). Funciones libres: `hqlJLiGw0gNN`, `LWCLfbEG`, `RrC7MAO` (func1/1.1), `XsSkLK`, `ZnW88I` (func1/1.1). 3 init closures + `init.init.func2.func3` (inusual = función anidada en init). Posición: DESPUÉS de `F5ImErztW`/`Q2v0Ja734`, ANTES de `mPmrATx3LG` (protobuf/impl) |
...
| `GztdjdTxnX1w` | **AC custom — paquete helper post-detección** | **12 funciones**. Tiene `init.0` (pre-init de variables de paquete). 2 init closures (func1-func2). Funciones: `AHT3Yy2mY`, `JTDbaa` (1 closure), `j2zCkeC4IXaN`, `pK8_3KNo`, `UUy_glV5C4nN`, `yxQYYm56XVw6` (1 closure). Sin `decFunc`. Posición EXACTA: DESPUÉS de `msvCchDB` (7mo registro de firmas), ANTES de `BSjj4d`. Pequeño paquete custom del AC, probablemente helper de procesamiento de resultados de detección |
| `h7lxVr` | **38** | **Wails internal** — entre CORS (`buwDgX7Zkn`) e `IUEIXURnge4` (Wails generics). Tiene `init.0`. Dos funciones nombradas en init: `nqtQbq` (closures func3/5/6/8) y `ujiGprDFR` (closures func4/7). Tipo principal `vWd_IjQBOX3G` (8 métodos: bPB7co6, btmIzv, cb5edY9Y, e7ci2EFo, eIoHcA, jNN2FgJQ9N, oNHwkW, u3FkR_swh). Tipos adicionales: `dXbuD0mAD0` (mETkVch9), `eIrUjTaYWX` (mETkVch9, qEaUwKReTS). Función libre con sub-métodos: `zrKJSRM.dGEak03It`/`sW2F0cAG8`. Posición: DESPUÉS de `NDWN1oE3`, ANTES de `IUEIXURnge4`+`Moh1QXpPW` (eventos Wails) |
...
| `D3c7OwSvpJ` | **34** | **Probable `google.golang.org/protobuf/encoding/protowire`** — codificación wire format protobuf | Tipo `Btf9ew8En` (IsValid) = `protowire.Number` (FieldNumber/TagNumber). 24 funciones libres todas garbled. 5 init closures con sub-closures (func2.1, func5.1). Sin `decFunc`. Posición: DESPUÉS de `GtN_WZiIQ`, ANTES de `yzULHj8` (protobuf/internal/impl codec). **Alias en type table: `t5W7d4Vh7E`** (mismo `Btf9ew8En` con IsValid, usado en FindExtensionByNumber) |
