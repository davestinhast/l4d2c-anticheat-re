...
| `cXI2MODy` | **Sub-paquete de Gin** (`gin/render` o `gin/binding`) — **154 funciones** | Tiene `decFunc` (garble -literals activo) y 115+ init closures (muchos string literals cifrados). Probablemente `gin/render` o `gin/binding` — sub-paquetes internos de Gin para rendering de respuestas y binding de request data |
...
| `FXIaelQr` | **Probable librería JWT** (`github.com/golang-jwt/jwt` o similar) | Evidencia: `JWTweH7OD.String` (tipo con nombre que contiene "JWT"). Funciones: `fb58lAbifZt` (2 closures), `pVFQlOq3cSxi` (3 closures), `ql04OezYep`, `vmQJdWB3XkC`, `z2ElcuDXPAbZ`, `iblYmRIo4`. Adyacente a `ogLSXbC1MZp` en pclntab. Si confirmado, el AC usa tokens JWT para autenticación de sesión |
