...
- `er9_yp` (56 funcs): 18 init closures + `decFunc` (garble -literals activo) → paquete AC custom pequeño; puede ser sub-módulo de blacklist con 18 patrones cifrados
- `w0bqp0RaeD` (125 funcs): **paquete de detección AC — inmediatamente anterior a `main` en pclntab**. 11 init closures. Función crítica `TnC04uk4`: 8 closures directas + `func6.1-16` + `func6.dhCJhjGa.16` + `gowrap1-6` = **6 goroutines lanzadas**. Funciones complejas: `QKaVdXPL` (8 closures), `aiV8LM9eoaot` (5), `bxCiu5XSx1` (5), `OY6DwAo` (5), `rut8pO1Cygha` (no closure count en esta sesión). 2 tipos comparables: `RIpbVXrKn` y `O1pxFVY` (usados como map keys). Sin `decFunc` (strings cleartext, but no registry paths found in clear). Probable: motor de monitoreo de procesos del juego u orquestador de detección pre-main. Vecinos: `GTShob4MX` (registry) → `T1XTTlVrw4yb` → `w0bqp0RaeD` → `main`
- `lI61JSyj` (30 funcs): ~10 init closures, adyacente a `HEirZZGS` (error types) → posible paquete helper con strings cifrados
- `DQTJuVp_jZ` (110 funcs): posición en pclntab entre `E5D2QU` (`internal/poll`) y `u26yruH2Ly_u` (`net` stdlib). Sin `decFunc`. **56 init closures** incluyendo `init.AAaUijjhdsR.func61/62` (función nombrada dentro de init) y `init.GFwvMRC3zna[go.shape.bool].func57-60` (función genérica parametrizada con `bool`). 9 tipos comparables: `E5Cq49a`, `NgA3XBFdmdb`, `OK6p4yyx`, `SgNrJN`, `TB3Q9SaVnrtE`, `TKS90l`, `VmoFu3c`, `XvC0liM9`, `ZHTCBMPJZQf`. Las 9 funciones top-level comparten nombre con los 9 tipos. Sin métodos non-garbled. Posible: **`net/netip`** (stdlib Go 1.18+ tipos IP) o **`github.com/go-playground/validator/v10`** sub-módulo. La posición entre `internal/poll` y `net` sugiere paquete stdlib en la cadena de dependencias de net
...
| `T1XTTlVrw4yb` | **Wails Windows utility adjacent to Toast** | 77 funciones. 19 init closures (string literals). Adyacente a `Kd1BZ5fvE` (Toast) y `aGxe6x3` (html/template). Probable paquete de runtime Wails para Windows |
...
| `fm9Lu8y5M2` | **Paquete custom AC** — 43 funciones | Todo garbled, no hay métodos preservados. Varios tipos complejos: `ezoeH8RzZQ`, `fbvyfXUwRs3`, `kfNqppH`, `t3m7x3OSoD`. Función `NjWooFOGaHB` (CamelCase) = función pública. Probable módulo de análisis o reporting del AC |
...
| `OeqkvWSN2RR` | 75 | Paquete custom AC. Tipo principal `(*wpuRTCttiJE)` con métodos garbled: `i0MmnFa3242T`, `jDXVcOt`, `mEj4Tr3nZ` (1 closure), `mmdV1Ykl` (2 closures), `xshtxE_s1vq` (1 closure). 23 funciones libres con closures anidados (d5kTRY6P6, tDwaBFGXx2xX, i1YkpWm5lmUW, mdg_prpe, hlm6xNZm, xnAwpTnm76UG). Sin `decFunc`. Vecino: `xzmWSJlaR` → `OeqkvWSN2RR`. Probable: módulo de conexión/sesión del AC |
